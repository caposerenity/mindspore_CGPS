...
def label_generator_dbscan_context_single(cfg, features, dist, eps, **kwargs):
    ...
    centers = [
        mindspore.ops.stack(centers[idx], dim=0).mean(0) for idx in sorted(centers.keys())
    ]
    centers = mindspore.ops.stack(centers, dim=0)
            ...
            for dup in dups:
                ...
                for i in range(len(sim)):
                    if i != idx:
                        ...
                        centers = mindspore.ops.cat((centers, features[tmp_id[dup[1][i]]]))
                        num_classes += 1
...
def label_generator_dbscan_context(cfg, features, cuda=True, indep_thres=None, all_inds=None, **kwargs):
    ...
    if len(eps) == 1:
        ...
        R_comp = 1 - mindspore.ops.min(label_sim, label_sim_tight).sum(-1) / mindspore.ops.max(
            label_sim, label_sim_tight
        ).sum(-1)
        R_indep = 1 - mindspore.ops.min(label_sim, label_sim_loose).sum(-1) / mindspore.ops.max(
