        ...
        for idx, (data_loader, dataset) in enumerate(
            ...
            if self.cfg.PSEUDO_LABELS.norm_feat:
                if isinstance(all_features, list):
                    all_features = [mindspore.ops.normalize(f, p=2, dim=1) for f in all_features]
                else:
                    all_features = mindspore.ops.normalize(all_features, p=2, dim=1)
            ...
            if self.rank == 0:
                ...
                if self.cfg.PSEUDO_LABELS.norm_center:
                    centers = mindspore.ops.normalize(centers, p=2, dim=1)
            ...
            if self.world_size > 1:
                ...
                if self.rank > 0:
                    labels = mindspore.ops.arange(len(dataset)).long()
                    centers = mindspore.ops.zeros((num_classes, all_features.size(-1))).float()
