...
class Faster_Rcnn_Resnet50(nn.Cell):
    ...
    def construct(self, img_data, img_metas, gt_bboxes, gt_labels, gt_valids):
        """Forward function of Faster R-CNN.

        Args:
            img_data: tensor of shape(N, C, H, W), N = #images
            img_metas: tensor of shape(N, 4), represents height, width, h_scale_factor, w_scale_factor of the images
            gt_bboxes: ground truth bounding boxes. set to None if not available.
            gt_labels: ground truth categories. set to None if not available.
            gt_valids: whether a ground truth box is valid. set to None if not available.

        Returns:
            tuple, containing the following 3 tensors:
                - the first one has shape [#images, #bboxes_per_image, 5], represents top-left and bottom-right 
                    x, y coordinates of the boxes in each images and their scores.
                - the second one has shape [#images, #bboxes_per_image, 1], represents categories of the boxes.
                - the third one has shape [#images, #bboxes_per_image, 1]. It's a bool tensor, represents which
                    boxes are valid.
        """
